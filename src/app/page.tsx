...
function HistoryView() {
  ...
  return <main className="view"><div className="page-heading"><div><span className="eyebrow">ÚLTIMOS 30 DIAS</span><h1>Histórico</h1><p>Acompanhe tudo que foi publicado e identifique falhas.</p></div><button className="secondary-button"><Share2 size={15} /> Exportar relatório</button></div><div className="history-filters"><button>Todos os clientes <ChevronDown size={14} /></button><button>Todos os estados <ChevronDown size={14} /></button><button>Últimos 30 dias <ChevronDown size={14} /></button><button className="filter-icon"><ListFilter size={15} /></button></div><section className="history-table"><div className="table-head"><span>CONTEÚDO</span><span>CLIENTE</span><span>DATA</span><span>FORMATO</span><span>STATUS</span><span /></div>{rows.map((row, index) => <div className="table-row" key={row.title}><div><div className="tiny-thumb"><Image src={media[index % media.length].src} alt="" fill sizes="44px" /></div><strong>{row.title}</strong></div><span>{row.client}</span><span>{row.date}</span><span>{row.kind}</span><span className={`status-pill ${statusClass[row.status]}`}><i />{row.status}</span><button aria-label={`Opções de ${row.title}`}><MoreHorizontal size={17} /></button></div>)}</section></main>;
}
